...
{
    return ((uint8_t)readByte(Addr,reg));
}
...
int BME280_id_Check() { //check if sensor is present, returns 0 if not
    ...
    BME280_id = I2Cread(BME280_ADDRESS, BME280_CHIP_ID_REG); //read BME280 ID byte, should be 0x60
...
int32_t read24(char address) { //reads in a 24bit value in Big Endlain
    int32_t value[4];
    value[0] = I2Cread(BME280_ADDRESS, address);
    value[1] = I2Cread(BME280_ADDRESS, address + 1);
    value[2] = I2Cread(BME280_ADDRESS, address + 2);
...
int read8(char address) { //reads in a 8bit value
    int value;
    value = I2Cread(BME280_ADDRESS, address);
    return value;
}
...
int BME280_Setup() { //setup the Sensor
    ...
    //I2Cwrite(BME280_ADDRESS, BME280_CTRL_HUMIDITY_REG, 0x05); //16x oversampling
    //I2Cwrite(BME280_ADDRESS, BME280_CONFIG_REG, 0xB7); // 16x ovesampling, normal mode
    

    //Setting Below are for 1 second sample rate.
    I2Cwrite(BME280_ADDRESS, BME280_CTRL_HUMIDITY_REG, 0x01); //normal mode
    I2Cwrite(BME280_ADDRESS, BME280_CTRL_MEAS_REG, 0b00100111); //normal mode
    I2Cwrite(BME280_ADDRESS, BME280_CONFIG_REG, 0xA0); //sample once per second
    return 1;
}
